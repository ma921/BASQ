...
class ScaleMmltGP(Utils):
    def __init__(
        ...
        self.kernel = self.gspace_kernel
        self.predict_mean = self.gspace_mean_predict
    ...
    def gspace_kernel(self, x, y):
        ...
        if len(mu_g_x.shape) == 1 and len(mu_g_y.shape) == 1:
            CLy = mu_g_x.unsqueeze(1) * mu_g_y.unsqueeze(0) * (cov_h_xy.exp() - 1)
        else:
            CLy = mu_g_x.unsqueeze(1) * mu_g_y.unsqueeze(1) * (cov_h_xy.exp() - 1)
